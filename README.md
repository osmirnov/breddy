...
<img src="breddy-logo.png" width="480" alt="B.RED.DY - The Defensive Planner Agent">
...
- **Result:** The agent _must_ handle these errors gracefully, demonstrating robust error recovery.
...
Includes a custom `MetricsCollector` and `@trace_tool` decorators to log every decision, tool call, and state change, proving that the _agents_ (not hardcoded logic) are in control.
